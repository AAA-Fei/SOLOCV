...
class YOLO_SEG:
    ...
    def postprocess(self, input_image: np.ndarray, outputs: List[np.ndarray],
        ...
        """
        适配带 NMS 的 seg 模型输出:
        output0 [1, 300, 38]: [x1, y1, x2, y2, score, class_id, mask_coefs(32)]
        output1 [1, 32, 160, 160]: 原型掩码
        坐标为输入尺寸(640×640)下的像素值（已做 letterbox）
        ...
        """
        dets = np.squeeze(outputs[0])  # (300, 38)
        protos = np.squeeze(outputs[1])  # (mask_dim, mask_h, mask_w)
        ...
        for row in dets:
            ...
            # 逆 letterbox 变换：减去填充偏移、除以缩放比例得到原图坐标
            x1 = (row[0] - pad[1]) / gain
            y1 = (row[1] - pad[0]) / gain
            x2 = (row[2] - pad[1]) / gain
            y2 = (row[3] - pad[0]) / gain
        ...
        detections = []
        if mask_coefs_list:
            masks_in_arr = np.stack(mask_coefs_list)
            masks = self.process_mask(protos, masks_in_arr, boxes_xyxy, pad, gain)

            for (bbox, score, class_id, mask) in zip(boxes_xyxy, scores, class_ids, masks):
                x1, y1, x2, y2 = bbox
                detections.append((x1, y1, x2, y2, class_id, score, mask))
                if self.draw_boxes:
                    box_xywh = [x1, y1, x2 - x1, y2 - y1]
                    self.draw_detections(input_image, box_xywh, score, class_id, mask)
...
if __name__ == "__main__":
    ...
    model = target_file / Path("models/seg/yolo-nms-seg-onnxruntime.onnx")
